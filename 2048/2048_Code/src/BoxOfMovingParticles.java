    ...
    public void moveup(){
        ...
        for(int j=1;j<ourbox[1].length-1;j++){
            for(int i=1;i<ourbox.length-1;i++){
                if(!" ".equals(ourbox[i][j]) && i>1){
                    int k = i;
                    mergecount++;
                    while(" ".equals(ourbox[k-1][j])){
                        moveAble=true;
    ...
    public void movedown(){
        ...
        for(int j=1;j<ourbox[1].length-1;j++){
            for(int i=ourbox.length-2;i>0;i--){
                if(!" ".equals(ourbox[i][j]) && i<ourbox.length-2){
                    int k = i;
                    mergecount++;
                    while(" ".equals(ourbox[k+1][j])){
                        moveAble=true;
    ...
    public void moveright(){
        ...
        for(int i=1;i<ourbox.length-1;i++){
            for(int j=ourbox[i].length-2;j>0;j--){
                if(!" ".equals(ourbox[i][j]) && j<ourbox[i].length-2){
                    int k = j;
                    mergecount++;
                    while(" ".equals(ourbox[i][k+1])){
                        moveAble=true;
